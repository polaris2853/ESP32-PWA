...
let isPainting = false; // Flag to track if the mouse/touch is being held down for continuous drawing.
...
for (let i = 0; i < 56; i++) {
	...
	pixel.addEventListener("mousedown", handleStart);
	pixel.addEventListener("touchstart", handleStart);
	pixel.addEventListener("mouseover", handleMove);
	pixel.addEventListener("touchmove", handleMove);

	grid.appendChild(pixel);
	pixels.push(pixel);
}

// Global mouse and touch up event: stops painting anywhere on the page.
document.body.addEventListener("mouseup", handleEnd);
document.body.addEventListener("touchend", handleEnd);

function handleStart(event) {
	isPainting = true;
	// For touch events, handle the initial color directly since the first touch doesn't trigger a move.
	if (event.type === 'touchstart') {
		...
		// For mouse events, the mousedown event can handle the initial coloring.
...
function handleMove(event) {
	if (isPainting) {
		...
		if (event.type.startsWith('touch')) {
			...
			const touch = event.touches[0];
			targetPixel = document.elementFromPoint(touch.clientX, touch.clientY);
		}
		if (targetPixel && targetPixel.classList.contains('pixel')) {
			targetPixel.style.backgroundColor = colorInput.value;
			saveFrame();
